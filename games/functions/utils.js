module.exports = {
  ...
  useMeBeforeEachRequest(game) {
    if (
      Server.ratelimitCounters[game.id].reqCounter >= game.ratelimit.request
    ) {
      Server.ratelimitCounters[game.id].reqCounter = 0;
      return new Promise(resolve =>
        setTimeout(
          () => resolve(Server.ratelimitCounters[game.id].reqCounter),
          game.ratelimit.every * 1000,
        ),
      );
  ...
  getGameDataDoc(data, tag_id, game_id, data_settings, game_account_info) {
    if (data === null || data === undefined || data === '') return null;
      ...
      return __logError(`[${tag_id}] data can't be an object`, data), null;
    else
      return {
        tag_id,
        game_id,
  ...
  async updateGameData(
    ...
    const document = this.getGameDataDoc(
      data,
      tag_id,
      game_id,
      data_settings,
    ...
    if (document == null) {
      // Delete null data
      await Server.fn.dbMethods.game_data
        .deleteByTagIDAndFilter(tag_id, {
          game_id,
          data_settings,
          game_account_info,
        })
        .catch(err =>
          __logError(`Can't delete null tag data : ${tag_id}`, err),
        );
    } else {
      // Check if data already exist
      await Server.fn.dbMethods.game_data
        .getByTagIDAndFilter(tag_id, {
          game_id,
          data_settings,
          game_account_info,
        })
        .then(async game_data => {
          if (game_data && game_data.length > 0) {
            ...
            await Server.fn.dbMethods.game_data
              ...
              .catch(err =>
                __logError(`Can't update game_data : ${tag_id}`, err),
              );
          } else {
            ...
            await Server.fn.dbMethods.game_data
              .insert(document)
              .catch(err =>
                __logError(`Can't insert game_data : ${tag_id}`, err),
              );
          }
        })
