...
module.exports.getAccountInfo = (gameID, settings) => {
  return new Promise((resolve, reject) => {
    Server.fn.game[gameID]
      .getUserByUsername(settings.username)
      .then(accountInfo => {
...
module.exports.getDataOneByOne = async (
  ...
  if (tag_ids.SPEEDRUN__PB__RANK || tag_ids.SPEEDRUN__PB__TIME) {
    ...
    const res = await Server.fn.game[game.id].getUserPersonalBests(
      game_account_info.user_id,
    );
  ...
  if (tag_ids.SPEEDRUN__ACCOUNT__USERNAME) {
    if (username) {
      ...
      const res = await Server.fn.game[game.id].getUserByID(
        game_account_info.user_id,
      );
