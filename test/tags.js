...
for (const game of games) {
  describe(`Game : ${game.game.name}`, () => {
    for (const tagIndex in game.tags) {
      ...
      const settings =
        game.tests[tag.id] && Array.isArray(game.tests[tag.id].settings)
          ? game.tests[tag.id].settings
          : [];
      const examples =
        game.tests[tag.id] && Array.isArray(game.tests[tag.id].examples)
          ? game.tests[tag.id].examples
          : [tag.exampleOriginal];
        ...
        for (const exampleIndex in examples) {
          describe(`Data : ${examples[exampleIndex]}`, () => {
            ...
            for (const setting of settings) {
              it(`With settings : ${JSON.stringify(
                setting.tagSettings,
              )}`, function() {
