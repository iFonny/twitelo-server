...
module.exports = {
  ...
  checkParamsPPTriggerIDs(params) {
    return new Promise((resolve, reject) => {
      ...
      if (params && Array.isArray(params)) return resolve(params);
        ...
        return reject(
          Server.fn.api.jsonError(400, 'Bad or Missing parameters'),
        );
    });
  },
  ...
  checkParamsLocale(params) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        config.constant.locales.includes(params.locale.toLowerCase())
      )
        return resolve(params.locale.toLowerCase());
  ...
  checkParamsSwitchStatus(params) {
    return new Promise((resolve, reject) => {
      // Check mandatory params
      if (typeof params.status && !isNaN(parseInt(params.status, 2)))
        return resolve(Boolean(parseInt(params.status, 2)));
  ...
  checkParamsSwitchNameStatus(params) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        !isNaN(parseInt(params.status, 2)) &&
  ...
  checkParamsEditPPTrigger(params) {
    return new Promise((resolve, reject) => {
      ...
      if (params.triggerID && typeof params.triggerID == 'string')
        ppTrigger.trigger_id = params.triggerID;
      else
        return reject(
          Server.fn.api.jsonError(400, 'Bad or Missing trigger ID'),
        );
      ...
      if (params.image && typeof params.image == 'string') {
        ...
        if (
          Server.base64.isBase64(params.image) &&
          (mime &&
            ['image/jpeg', 'image/gif', 'image/png'].includes(mime[1])) &&
          fileSize <= config.media.maxSize
        )
          ppTrigger.image = params.image;
        else
          return reject(Server.fn.api.jsonError(400, 'Bad image or bad size'));
      }
      resolve(ppTrigger);
    });
  },
        ...
        .then(total => {
          if (total < config.constant.limits.ppTrigger) {
              ...
              .then(result =>
                resolve(
                  Server.fn.api.jsonSuccess(
                    200,
                    ((result.changes[0].new_val.game = null),
              ...
              .catch(err =>
                reject(
                  Server.fn.api.jsonError(
                    500,
                    'Internal server error',
                    '[DB] createEmptyPPTriggerSetting() error',
                    err,
                  ),
                ),
              );
            ...
            reject(
              Server.fn.api.jsonError(
                405,
                `Can't create more ppTrigger (limit: ${
                  config.constant.limits.ppTrigger
                })`,
                `Can't create more ppTrigger (limit: ${
                  config.constant.limits.ppTrigger
                })`,
    ...
    return new Promise((resolve, reject) => {
      if (ppTrigger.image) {
        Server.makeDir(`${config.root}${config.media.path.pp}/${userID}`) // Create dir if doesn't exist
          .then(path => {
            const filepath = Server.base64.utils.imgSync(
              ppTrigger.image,
              path,
              ppTrigger.id,
            ); // Save base64 image
            ppTrigger.path = filepath.replace(
              `${config.root}${config.media.path.root}`,
              '',
            );
          ...
          .catch(err =>
            reject(
              Server.fn.api.jsonError(
                500,
                'Internal server error',
                '[DB] updatePPTrigger() image error',
                err,
              ),
            ),
          );
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              'Internal server error',
              '[DB] updateUserGlobalSwitch() error',
              err,
            ),
          ),
        );
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              'Internal server error',
              '[DB] updateNotificationSwitch() error',
              err,
            ),
          ),
        );
  ...
  deletePPTriggers(userID, IDs) {
    return new Promise((resolve, reject) => {
      Server.fn.dbMethods.setting
        .getAllWithPathByIDs(userID, IDs)
  ...
  deletePPTriggerImages(ppTriggers) {
    return new Promise(resolve => {
      for (let ppTrigger of ppTriggers) {
        try {
          Server.fs.unlinkSync(
            `${config.root}${config.media.path.root}${ppTrigger.path}`,
          );
        } catch (error) {
  ...
  formatSettings(settings) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const triggers = _.keyBy(
          await Server.fn.dbMethods.trigger.getAll(),
          'id',
        );
        ...
        for (const key in formatedSettings) {
          for (const settingKey in formatedSettings[key]) {
            formatedSettings[key][settingKey].game = triggers[
              formatedSettings[key][settingKey].trigger_id
            ]
