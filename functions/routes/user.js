...
module.exports = {
  ...
  checkParamsSaveProfile(params) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof params.description == 'string')
        ...
        return reject(
          Server.fn.api.jsonError(400, 'Bad or Missing description'),
        );

      if (typeof params.location == 'string')
        profile.location = params.location;
      else
        return reject(Server.fn.api.jsonError(400, 'Bad or Missing location'));
          ...
          if (!error) {
            if (twUser)
              return resolve({
                ...
                lang: twUser.lang,
                profile_image_url: twUser.profile_image_url_https.replace(
                  '_normal',
                  '_400x400',
                ),
              ...
              return reject(
                Server.fn.api.jsonError(
                  500,
                  'Internal server error',
                  `[Twitter] Can't get credentials for @${user.username} (${
                    user.twitter_id
                  }) (no twUser)`,
                  twUser,
                ),
  ...
  updateIncludedTags(user, data) {
    return new Promise(async (resolve, reject) => {
      // Get updated user
      if (data)
        user = await Server.fn.dbMethods.user
          .get(user.id)
          .catch(err =>
            reject(
              Server.fn.api.jsonError(
                500,
                "Can't get user",
                'get user updateIncludedTags() error',
                err,
              ),
            ),
          );
      ...
      let descriptionTags = this.getIncludedTagsFromText(
        user.twitelo.description.content,
      );
      let locationTags = this.getIncludedTagsFromText(
        user.twitelo.location.content,
      );

      if (!data) {
        let nameSize = await this.getProfileTextLength(
          user.id,
          user.twitelo.name.content,
          nameTags,
        ).catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't get name size",
              'getProfileTextLength() error',
              err,
            ),
          ),
        );
        let descriptionSize = await this.getProfileTextLength(
          user.id,
          user.twitelo.description.content,
          descriptionTags,
        ).catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't get description size",
              'getProfileTextLength() error',
              err,
            ),
          ),
        );
        let locationSize = await this.getProfileTextLength(
          user.id,
          user.twitelo.location.content,
          locationTags,
        ).catch(err =>
        ...
        if (nameSize > config.constant.twitterLimits.name)
          reject(Server.fn.api.jsonError(400, 'Too many characters in name'));
        if (descriptionSize > config.constant.twitterLimits.description)
          reject(
            Server.fn.api.jsonError(400, 'Too many characters in description'),
          );
        if (locationSize > config.constant.twitterLimits.location)
          reject(
            Server.fn.api.jsonError(400, 'Too many characters in location'),
          );
      }

      if (user.switch && user.disabled < config.constant.disabledAfter) {
        if (user.twitelo.name.status)
          includedTags = _.concat(includedTags, nameTags);
        if (user.twitelo.description.status)
          includedTags = _.concat(includedTags, descriptionTags);
        if (user.twitelo.location.status)
          includedTags = _.concat(includedTags, locationTags);
      }
      ...
      await Server.fn.dbMethods.tag
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't update user's tags",
              '[DB] updateIncludedTags() error',
              err,
            ),
          ),
        );
      ...
      await Server.fn.dbMethods.tag
        .updateByUserAndIDs(user.id, includedTags, {
          included: true,
        })
        .catch(err =>
          reject(
      ...
      if (data) resolve(data);
        ...
        Server.fn.dbMethods.tag
          .getByUserAndIDs(user.id, allTags)
          .then(tags =>
            resolve({
              tags,
              profile: {
  ...
  getIncludedTags(userID, profile) {
    return new Promise(async (resolve, reject) => {
      ...
      // Get all tags in profile
      Server.fn.dbMethods.tag
        .getByUserAndIDs(userID, allTags)
        .then(tags =>
          resolve({
            tags,
            profile,
  ...
  getPreview(tags, profile, forTwitter) {
      ...
      while (match != null) {
        ...
        if (tag && tag.data) {
          try {
            ...
            generator =
              Server.gameAPI[tag.game_id].generator[gameTag.generator];
            if (forTwitter)
              mapObj[`<{${match[1]}}>`] = generator(
                gameTag,
                tag.data,
                tag.settings,
              );
            ...
            __logError(
              `Error with generator or gameTag \`${tag.tag_id}\``,
              error,
            );
            mapObj[`<{${match[1]}}>`] = '';
          }
    ...
    profile.description =
      getProfileTextPreview(profile.description, tags, forTwitter) || '';
    profile.location =
      getProfileTextPreview(profile.location, tags, forTwitter) || '';

    if (forTwitter) {
      profile.name = profile.name.substr(0, config.constant.twitterLimits.name);
      profile.description = profile.description.substr(
        0,
        config.constant.twitterLimits.description,
      );
      profile.location = profile.location.substr(
        0,
        config.constant.twitterLimits.location,
      );
    }
            ...
            if (err)
              reject(
                Server.fn.api.jsonError(
                  500,
                  'Internal server error',
                  '[TWITTER] getUpdatedTwitterUser() error',
                  err,
                ),
              );
            else {
