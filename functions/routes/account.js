...
module.exports = {
  ...
  checkParamsAccountCreate(params) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        Server.gameAPI[params.game_id]
      )
        account.game_id = params.game_id;
      else
  ...
  checkParamsAccountUpdateSettings(bodyParams, urlParams) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        account.id = urlParams.id;
      else return reject(Server.fn.api.jsonError(400, 'Bad or Missing id'));

      if (
        bodyParams.game_id &&
        typeof bodyParams.game_id == 'string' &&
        Server.gameAPI[bodyParams.game_id]
      )
        account.game_id = bodyParams.game_id;
      else
        return reject(Server.fn.api.jsonError(400, 'Bad or Missing game_id'));
      ...
      if (bodyParams.settings && Server.gameSettings[account.game_id]) {
        for (const key in Server.gameSettings[account.game_id]) {
          if (
            ...
            if (key != 'verify')
              account.settings[key] = bodyParams.settings[key];
          } else
            return reject(
              Server.fn.api.jsonError(400, `Bad or Missing '${key}' setting`),
            );
        ...
        .then(info => {
          if (info)
            resolve(
              ...
              (account.game_account_info = info),
            ...
            reject(
              Server.fn.api.jsonError(
                ...
                user.settings.locale == 'fr'
                  ? "Le compte n'est pas valide ou n'existe pas."
                  : 'Account invalid or does not exist.',
              ),
            );
        })
        .catch(err =>
        ...
        .then(total => {
          if (total < config.constant.limits.accountbyGame) {
            ...
            reject(
              Server.fn.api.jsonError(
                405,
                `Can't add more accounts (limit: ${
                  config.constant.limits.accountbyGame
                })`,
                `Can't add more accounts (limit: ${
                  config.constant.limits.accountbyGame
                })`,
            ...
            Server.fn.dbMethods.tag
              ...
              .catch(() =>
                reject(
                  Server.fn.api.jsonError(
                    500,
                    "Can't update account : tags not found",
                  ),
                ),
              );
      ...
      Server.fn.dbMethods.tag
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't get tags to delete",
              '[DB] getTagsToDelete() error',
              err,
            ),
          ),
        );
  ...
  deleteTagsFromProfile(user, data) {
    return new Promise((resolve, reject) => {
      ...
      const twitelo = {
        description: {
          content: removeFromProfile(
            user.twitelo.description.content.trim(),
            ids,
          ),
        },
        name: {
      ...
      Server.fn.dbMethods.user
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't delete tags from profile",
              '[DB] deleteTagsFromProfile() error',
              err,
            ),
          ),
        );
      ...
      Server.fn.dbMethods.tag
        ...
        .catch(err =>
          reject(
            Server.fn.api.jsonError(
              500,
              "Can't delete tags or account",
              '[DB] deleteTagsAndAccount() error',
              err,
            ),
          ),
        );
