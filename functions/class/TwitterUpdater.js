...
module.exports = class TwitterUpdater {
  ...
  update() {
    ...
    return Server.fn.dbMethods.user
      .getUsersToUpdate()
      ...
      .then(() => {
        ...
        __log(
          `__**Twitter**__ - update finished in **${elapsedTimeS}s ${elapsedTimeMS}ms**`,
        );
        return __logRecapTwitter(
          `__**Twitter**__ - ${this.total.updated} users updated ${
            this.total.notUpdated
              ? `(${this.total.notUpdated} not updated) `
              : ''
          }in **${elapsedTimeS}s**`,
        );
      })
      .catch(err =>
        __logError("[DB] twitterUpdater - Can't get users to update", err),
      );
  }
  ...
  async updateUsers(users) {
    try {
      for (let user of users) {
        ...
        await this.getTwitterProfileToUpdate().then(
          profile =>
            profile ? this.updateTwitterProfile(profile, twitterUser) : null,
        );
  ...
  connectToTwitter() {
    ...
    this.user.tokens = Server.jwt.decode(
      this.user.tokens,
  ...
  getTwitterProfileToUpdate() {
    // Get twitter profile
    return Server.fn.routes.user
      .getIncludedTags(this.user.id, {
      ...
      .then(data =>
        Server.fn.routes.user.getPreview(data.tags, data.profile, true),
      )
      .then(data => data.data)
      .then(profile => {
        // Return null if nothing activated
        if (
          !this.user.twitelo.name.status &&
          !this.user.twitelo.description.status &&
          !this.user.twitelo.location.status
        )
          return null;
    ...
    return Server.fn.dbMethods.user
      .update(this.user.id, {
        ...
        profile_image_url: twUser.profile_image_url_https
          ? twUser.profile_image_url_https.replace('_normal', '_400x400')
          : 'https://twitelo.me/images/errors/default_profile.png',
      ...
      .catch(err =>
        Promise.resolve(
          __logError("[DB] profileUpdated - Can't update user", err),
        ),
      );
  }
  ...
  profileNotUpdated(error) {
    ...
    if (error && error.code) {
      switch (error.code) {
        ...
        default:
          return Promise.resolve(
            __logError(
              `[Unknown] Can't update user @${this.user.username} (code: ${
                error.code
              })`,
              error,
            ),
      ...
      return Promise.resolve(
        __logWarning(
          `[Twitter] Timeout on @${this.user.username} - ${error.message}`,
          error,
        ),
      );
    } else
      return Promise.resolve(
        __logError(
          `[Unknown] Twitter update error @${this.user.username}`,
          error,
        ),
      );
  }
  ...
  twiteloAppRevoked(error) {
    ...
    if (this.user.disabled >= config.constant.disabledAfter) {
      __logError(
        `Can't update user @${this.user.username} (code: ${
          error.code
        }) - Now disabled`,
        error,
      );
      return Server.fn.dbMethods.user
        ...
        .then(() =>
          Server.fn.dbMethods.notification.sendNotification(
            this.user.id,
            'exclamation-circle',
            'error',
            {
              en: {
                title: 'Twitter error',
                content: `Twitelo no longer has access to your account. If you want to continue using twitelo, please log in again (${
                  config.server.appURL
                }).`,
              },
              fr: {
                title: 'Erreur twitter',
                content: `Twitelo n'a plus accès a votre compte. Si vous voulez continuer d'utiliser twitelo, veuillez vous reconnecter (${
                  config.server.appURL
                }).`,
              },
            },
        ...
        .catch(err =>
          __logError(
            "[DB] twitterUpdater - Can't update user/updateTags/sendNotif",
            err,
          ),
        );
    } else {
      return Server.fn.dbMethods.user
        ...
        .catch(err =>
          __logError("[DB] twitterUpdater - Can't update user", err),
        );
    }
  }
  ...
  profileUpdateFailed(error) {
    __logError(
      `Can't update user @${this.user.username} (code: ${error.code})\n` +
        `- Name: ${this.user.twitelo.name.content}\n` +
        `- Description: ${this.user.twitelo.description.content}\n` +
        `- Location: ${this.user.twitelo.location.content}`,
      error,
        ...
        Server.fn.dbMethods.notification.sendNotification(
          ...
          'error',
          {
            en: {
              title: 'Twitter error',
              content:
                'Unable to update your profile, the updater has been disabled. Please apply the necessary corrections before reactivating it.\n\n' +
                error.message,
            },
            fr: {
              title: 'Erreur twitter',
              content:
                "Impossible de mettre à jour votre profil, l'updater a donc été desactivé. Veuillez appliquer les corrections nécessaires avant de le réactiver.\n\n" +
                error.message,
            },
          },
        ),
