module.exports = {
  ...
  router: function(express, routeName) {
        ...
        Server.fn.routes.tag
          ...
          .then(data =>
            __logUserAction(
              `__${routeName}__ - **@${
                req.user.username
              }** vient d'ajouter :heavy_plus_sign: un tag : \`${
                data.data.tag_id
              }\``,
              data,
            ),
    ...
    routerMe.post(
      ...
      (req, res) => {
        Server.fn.routes.tag
          .checkParamsTagUpdateSettings(req.body, req.params)
          .then(tag => Server.fn.routes.tag.updateTagSettings(req.user.id, tag))
          .then(data =>
            __logUserAction(
              `__${routeName}__ - **@${
                req.user.username
              }** vient d'editer :pencil: un tag : \`${req.body.tag_id}\``,
              data,
            ),
          )
          .then(data => res.status(data.status).json(data))
