  ...
  router: function(express, routeName) {
    ...
    routerMe.put(
      ...
      (req, res) => {
        Server.fn.routes.account
          ...
          .then(account =>
            Server.fn.routes.account.getAccountID(req.user, account),
          )
          ...
          .then(data =>
            __logUserAction(
              `__${routeName}__ - **@${
                req.user.username
              }** vient d'ajouter :heavy_plus_sign: un compte **${
                data.data.game_id
    ...
    routerMe.post(
      ...
      (req, res) => {
        Server.fn.routes.account
          ...
          .then(account =>
            Server.fn.routes.account.updateAccountSettings(
              req.user.id,
              account,
            ),
          )
          .then(data =>
            __logUserAction(
              `__${routeName}__ - **@${
                req.user.username
              }** vient d'editer :pencil: un compte **${
                req.body.game_id
    ...
    routerMe.delete('/:id/delete', (req, res) => {
      Server.fn.routes.account
        ...
        .then(data =>
          Server.fn.routes.account.deleteTagsFromProfile(req.user, data),
        )
        .then(data =>
          Server.fn.routes.account.deleteTagsAndAccount(
            req.user.id,
            data.account,
            data.tags,
          ),
        ...
        .then(data =>
          __logUserAction(
            `__${routeName}__ - **@${
              req.user.username
            }** a supprimé un compte et tous les tags liés`,
            data,
          ),
        )
        .then(data => res.status(data.status).json(data))
