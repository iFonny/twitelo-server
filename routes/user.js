module.exports = {
  ...
  router: function(express, routeName) {
    ...
    routerMe.get('/update', (req, res) => {
      Server.fn.routes.user
        .getTwitterUserData(req.user)
        .then(twitterData =>
          Server.fn.routes.user.updateUser(req.user.id, twitterData),
        )
    ...
    routerMe.post(
      ...
      (req, res) => {
        Server.fn.routes.user
          .checkParamsSaveProfile(req.body)
          .then(profile =>
            Server.fn.routes.user.updateProfile(req.user, profile),
          )
          .then(user => Server.fn.routes.user.updateIncludedTags(user))
          .then(data =>
            Server.fn.routes.user.getPreview(data.tags, data.profile, false),
          )
          .then(data =>
            __logUserAction(
              `__${routeName}__ - **@${
                req.user.username
              }** vient de sauvegarder son profil :pencil:`,
              data,
            ),
          )
          .then(data => res.status(data.status).json(data))
    ...
    routerMe.post(
      ...
      (req, res) => {
        Server.fn.routes.user
          .checkParamsSaveProfile(req.body)
          .then(profile =>
            Server.fn.routes.user.getIncludedTags(req.user.id, profile),
          )
          .then(data =>
            Server.fn.routes.user.getPreview(data.tags, data.profile, false),
          )
    ...
    routerMe.delete('/delete', (req, res) => {
      Server.fn.routes.user
        .getUser(req.user.id)
        .then(user => Server.fn.routes.user.deleteUserData(user.data))
        .then(user => Server.fn.routes.user.deleteUser(user))
        .then(data =>
          __logUserAction(
            `__${routeName}__ - **@${
              req.user.username
            }** vient de supprimer son compte :wastebasket:`,
            data,
          ),
        )
