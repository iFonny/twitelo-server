module.exports = {
  ...
  router: function(express, routeName) {
    ...
    routerMe.get('/all', (req, res) => {
      Server.fn.routes.notification
        .getAllNotifications(req.user.id)
    ...
    routerMe.get('/all/limit/:limit', (req, res) => {
      Server.fn.routes.notification
        .checkParamsNotificationLimit(req.params)
        .then(limit =>
          Server.fn.routes.notification.getAllNotifications(req.user.id, limit),
        )
        .then(data => res.status(data.status).json(data))
        .catch(err => res.status(err.status).json(err));
    });
    ...
    routerMe.get('/unarchived', (req, res) => {
      Server.fn.routes.notification
        .getUnarchivedNotifications(req.user.id)
        .then(data => res.status(data.status).json(data))
        .catch(err => res.status(err.status).json(err));
    });
    ...
    routerMe.get('/unarchived/limit/:limit', (req, res) => {
      Server.fn.routes.notification
        .checkParamsNotificationLimit(req.params)
        .then(limit =>
          Server.fn.routes.notification.getUnarchivedNotifications(
            req.user.id,
            limit,
          ),
        )
    ...
    routerMe.post('/archive', (req, res) => {
      Server.fn.routes.notification
        .checkParamsNotificationIDs(req.body)
        .then(notificationIDs =>
          Server.fn.routes.notification.archiveNotifications(
            req.user.id,
            notificationIDs,
          ),
        )
        .then(data =>
          __logUserAction(
            `__${routeName}__ - **@${
              req.user.username
            }** vient d'archiver des notifications`,
            data,
          ),
        )
        .then(data => res.status(data.status).json(data))
    ...
    routerMe.post('/unarchive', (req, res) => {
      Server.fn.routes.notification
        .checkParamsNotificationIDs(req.body)
        .then(notificationIDs =>
          Server.fn.routes.notification.unarchiveNotifications(
            req.user.id,
            notificationIDs,
          ),
        )
        .then(data =>
          __logUserAction(
            `__${routeName}__ - **@${
              req.user.username
            }** vient de désarchiver des notifications`,
            data,
          ),
        )
        .then(data => res.status(data.status).json(data))
